...
import { Product } from '../product/product.module'; // Assurez-vous que le chemin est correct
...
export class ProductsCategoryComponent implements OnInit {
  ...
  ngOnInit() {
    // Récupération de l'ID de catégorie et filtrage des produits
    this.ac.paramMap.subscribe((res) => {
      this.id = Number(res.get('id')); // Récupération de l'ID depuis l'URL
      this.list = this.listProducts.filter((product) => product.categoryId === this.id); // Filtrage des produits par catégorie
    });
  }
}
